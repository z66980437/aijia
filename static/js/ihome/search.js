...
// 分页
function paginate(page,pages,iter_pages, aid) {
    var search_html = template('search_panigate_script',{pages: pages, page:page, iter_pages:iter_pages, aid:aid})
     $('.pagination').html(search_html)
}
    ...
    $.ajax({
        ...
        success:function(data){
            if(data.code == '200'){
                ...
                paginate(data.paginate_page, data.paginate_pages, data.iter_pages, aid=data.aid);
            }
        }
    })
}
...
$(document).ready(function(){
    ...
    $.get('/house/area_facility/', function(data){
        if(data.code == '200'){
            var area_html = template('search_area_script',{areas:data.area});
